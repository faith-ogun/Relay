// ── Ohmlet circuit engine ──
//
// Shared verbatim with frontend/components/ohmlet/sim/engine.ts. It is pure
// TypeScript with no DOM, no React and no browser API, which is why the port
// is a copy rather than a rewrite: the physics has no business differing
// between a phone and a browser, and two implementations would drift.
//
// Keep the two in step. If this file and the web one disagree, the same
// circuit teaches two different lessons depending on which surface a learner
// happens to open.
//
