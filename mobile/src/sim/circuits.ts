...
export interface LiveControl {
  label: string;
  /**
   * How travel maps to value. Resistance is ratiometric, so LOG is the honest
   * default: doubling matters the same amount whether you are at 500 ohms or
   * 50k. Mapped linearly, a 500 to 47k divider put every useful value in the
   * first fifth of the track and left the rest doing nothing you could see.
   */
  scale?: 'linear' | 'log';
...
export const LIVE_CIRCUITS: LiveCircuitDef[] = [
  {
    ...
    control: { label: 'Resistor', unit: 'Ω', min: 47, max: 4700, step: 1, initial: 220, scale: 'log', format: ohms },
    build: (r) => [
  ...
  },
  {
    ...
    control: { label: 'Lower resistor', unit: 'Ω', min: 500, max: 47000, step: 10, initial: 10000, scale: 'log', format: ohms },
    build: (r2) => [
  ...
  },
  {
    ...
    control: { label: 'Second branch', unit: 'Ω', min: 220, max: 10000, step: 10, initial: 2000, scale: 'log', format: ohms },
    build: (r2) => [
  ...
  },
  {
    ...
    control: { label: 'Resistor', unit: 'Ω', min: 1000, max: 100000, step: 10, initial: 10000, scale: 'log', format: ohms },
    build: (r) => [
      { kind: 'V', id: 'src', pos: 1, neg: 0, value: 5 },
  ...
  },
  {
    ...
    control: { label: 'Base resistor', unit: 'Ω', min: 1000, max: 470000, step: 10, initial: 10000, scale: 'log', format: ohms },
    build: (rb) => [
