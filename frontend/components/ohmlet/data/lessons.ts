...
export const LESSON_CONTENT: Record<string, { steps: AuthoredStep[]; xpReward: number }> = {
  ...
  'DC Motors and Drivers': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 1, question: 'Why can you not wire a DC motor directly to an Arduino output pin?', options: ['It draws too much current and produces a back-EMF spike', 'It runs at a much lower frequency than the pin can match', 'The pin outputs AC and the motor needs DC to turn', 'A motor only works when wired to an analog input pin'], correct: 0, explanation: 'The motor needs more current than a pin can source, and its inductance kicks back a voltage spike when switched off.' },
      ...
      { type: 'multiple_choice', difficulty: 2, question: 'A gearbox on a small DC motor is mainly used to...', options: ['Trade higher speed for more torque', 'Convert the motor from DC to AC operation', 'Remove the need for a flyback diode', 'Let the motor run on a lower voltage'], correct: 0, explanation: 'Gearing reduces output speed while multiplying torque, which is why robot wheels use geared motors.' },
  ...
  'Powering Motors Safely': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 2, question: 'Sizing a motor supply, the current you must plan for is the...', options: ['Stall current, which is far higher than the running current', 'Idle current measured with the motor switched off', 'Logic current the Arduino itself draws', 'Leakage current through the flyback diode'], correct: 0, explanation: 'Stall (rotor held) draws the most current, so the supply and driver must survive it.' },
  ...
  'The H-Bridge': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 1, question: 'How many switches does an H-bridge use?', options: ['Four', 'One', 'Two', 'Eight'], correct: 0, explanation: 'Four switches form the H, with the motor in the crossbar.' },
      ...
      { type: 'predict_behavior', difficulty: 2, question: 'All four H-bridge switches are open. The motor...', options: ['Coasts freely, with no current driven through it', 'Brakes hard and stops at once because both sides short', 'Spins forward at full speed', 'Shorts the supply dangerously'], correct: 0, explanation: 'With every switch open there is no path, so the motor is driven by nothing and coasts.' },
  ...
  'PWM Motor Speed Control': {
    ...
    steps: [
      ...
      { type: 'predict_reading', difficulty: 2, question: 'analogWrite(64) on a 12V motor supply gives roughly what duty cycle and average voltage?', options: ['About 25 percent, so about 3 V', 'About 50 percent, so about 6 V', 'About 75 percent, so about 9 V', 'About 100 percent, so about 12 V'], correct: 0, explanation: '64/255 ≈ 0.25, and 0.25 × 12V = 3V.' },
  ...
  'Servo Motors': {
    ...
    steps: [
      ...
      { type: 'predict_behavior', difficulty: 2, question: 'A 1 ms pulse every 20 ms commands the servo to about...', options: ['0 degrees', '45 degrees', '90 degrees', '180 degrees'], correct: 0, explanation: 'The shortest standard pulse (1 ms) maps to one travel limit, about 0 degrees.' },
      { type: 'multiple_choice', difficulty: 2, question: 'Across the standard 1 ms to 2 ms pulse range, the servo angle covers roughly...', options: ['0 to 180 degrees', '0 to 90 degrees', '0 to 360 degrees', '90 to 180 degrees'], correct: 0, explanation: '1 ms is one end (0 deg), 2 ms the other (180 deg), so the span is about 180 degrees.' },
  ...
  'Stepper Motors': {
    ...
    steps: [
      ...
      { type: 'predict_reading', difficulty: 2, question: 'How many steps does a 200-step-per-rev motor need for one full revolution?', options: ['200', '100', '360', '400'], correct: 0, explanation: 'By definition, 200 steps per revolution means 200 steps for one full turn.' },
  ...
  'Distance Sensing': {
    ...
    steps: [
      ...
      { type: 'predict_reading', difficulty: 2, question: 'Speed of sound 340 m/s. The echo returns 10 ms after the pulse is sent. How far is the object? Remember the round trip.', options: ['1.7 m', '3.4 m', '0.85 m', '34 m'], correct: 0, explanation: 'Round trip = 340 × 0.01 = 3.4 m; the object is half of that, 1.7 m.' },
  ...
  'Talking to Modules': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 2, question: 'A fast display needs the highest data rate. The bus to reach for is...', options: ['SPI, the fastest of the three', 'I2C, limited by its pull-ups and bus capacitance', 'UART, with no clock to keep sync', 'None, displays cannot use a bus'], correct: 0, explanation: 'SPI runs fastest, which suits a high-throughput display.' },
  ...
  'The Robot Control Loop': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 2, question: 'A slow push-button that starts the robot is best handled by...', options: ['Polling it in the main loop', 'A high-priority hardware interrupt', 'An I2C address lookup', 'A flyback diode'], correct: 0, explanation: 'Slow, infrequent inputs are perfectly fine to poll; interrupts are for fast or critical events.' },
  ...
  'Closing the Loop with Feedback': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 2, question: 'Which scenario is genuinely closed-loop?', options: ['Read the wheel encoder, compare to target, adjust the motor', 'Run the motor at a fixed duty for a fixed time', 'Send the same pulse to a servo and stop sensing', 'Drive forward and hope the distance is right'], correct: 0, explanation: 'Only the first measures the result and corrects, which is the closed-loop definition.' },
  ...
  'The Line-Follower': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', difficulty: 2, question: 'Both line sensors read clearly over the light floor (above threshold). The robot has...', options: ['Lost the line, and should search rather than drive straight on', 'Found the line perfectly and should speed up', 'A flat battery, the only possible cause', 'A shorted H-bridge that must be reset'], correct: 0, explanation: 'No sensor sees the line, so it has run off; the right response is a recovery search, not blindly continuing.' },
      { type: 'predict_reading', difficulty: 2, question: 'On a 0 to 1023 analog scale, the line reads ~200 and the floor reads ~800. A sensible threshold value is about...', options: ['500, roughly midway between the two', '1000, just below the maximum', '50, just above the minimum', '850, above the floor reading'], correct: 0, explanation: 'A threshold midway between line (200) and floor (800) cleanly separates the two; 500 fits.' },
      ...
      { type: 'multiple_choice', difficulty: 3, question: 'Why might a line-follower oscillate, wobbling left-right across the line instead of tracking it smoothly?', options: ['It over-corrects: each hard turn overshoots, so it keeps swinging past centre', 'The line is far too dark for any kind of IR sensor to ever detect it', 'The control loop runs far too slowly to ever react', 'The H-bridge is permanently stuck in the brake state'], correct: 0, explanation: 'Crude full-on turns overshoot the centre and trigger an opposite full turn, so it weaves; gentler, proportional correction settles it.' },
  ...
  'Unit 12 Checkpoint': {
    ...
    steps: [
      ...
      { type: 'multiple_choice', question: 'A separate motor supply must still share which connection with the Arduino?', options: ['A common ground', 'The 5V logic rail', 'A chip-select line', 'The SDA data line'], correct: 0, explanation: 'A shared 0V reference lets the control signals mean the same thing to both sides.' },
      { type: 'multiple_choice', question: 'An H-bridge mainly lets you...', options: ['Reverse a motor\'s direction', 'Convert AC to DC', 'Limit the LED current', 'Read a sensor address'], correct: 0, explanation: 'Four switches steer current either way through the motor, controlling direction.' },
      { type: 'predict_reading', question: 'A motor on a 9V supply driven at 50 percent PWM duty sees an average voltage of about...', options: ['4.5 V', '9 V', '18 V', '0 V'], correct: 0, explanation: 'Average = duty × supply = 0.5 × 9 = 4.5V.' },
      ...
      { type: 'predict_reading', question: 'Speed of sound 340 m/s; an ultrasonic echo returns after 10 ms. The object distance is...', options: ['1.7 m', '3.4 m', '0.85 m', '34 m'], correct: 0, explanation: 'Round trip 340 × 0.01 = 3.4 m; halve it for 1.7 m.' },
