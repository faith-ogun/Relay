...
export const InterviewReportView: React.FC<Props> = ({ report, onRetry, onOpenLessons }) => {
  const r = report;
  // The server routes each weakness to a real skill and marks the ones it could
  // not place. Both halves are shown: one is where to go next, the other is an
  // honest admission that we have no lesson for it.
  const topics = r.recommendedTopics ?? [];
  const covered = topics.filter((t) => t.covered && t.skillId);
  const uncovered = [...(r.uncoveredTopics ?? []), ...topics.filter((t) => !t.covered).map((t) => t.topic)]
          ...
          <section className="rounded-2xl border-2 border-ohmlet-ink bg-ohmlet-surface p-5 shadow-press-sm">
            ...
            {covered.length > 0 ? (
              <ul className="mt-3 space-y-2">
                {covered.map((t, i) => (
                  <li key={i} className="rounded-xl border-2 border-ohmlet-line bg-ohmlet-cream px-3 py-2">
                    <p className="text-sm font-extrabold leading-snug text-ohmlet-ink">{t.topic}</p>
                    <p className="mt-0.5 text-xs font-bold text-ohmlet-ink-soft">
                      {t.skillTitle}
                      {t.unitTitle ? ` · ${t.unitTitle}` : ''}
                    </p>
                    {t.why && <p className="mt-1 text-xs font-semibold leading-snug text-ohmlet-ink-soft">{t.why}</p>}
                  </li>
                ))}
              </ul>
            ) : (
            ...
            {/* Said out loud rather than hidden. Being told you were weak on
                something and quietly given no way to fix it is worse than being
                told plainly that the lesson does not exist. */}
            {uncovered.length > 0 && (
              <div className="mt-4 rounded-xl border-2 border-dashed border-ohmlet-line px-3 py-2.5">
                <p className="text-[11px] font-black uppercase tracking-wide text-ohmlet-ink-soft">
                  Asked about, not taught here yet
                </p>
                <p className="mt-1 text-xs font-semibold leading-snug text-ohmlet-ink-soft">
                  {uncovered.join(' · ')}
                </p>
              </div>
            )}

            {onOpenLessons && covered.length > 0 && (
              <button
                type="button"
