...
const INTERACTIVE = new Set(['match', 'drag_order', 'identify_component', 'spot_error', 'draw_connection', 'trace_current', 'fix_the_circuit', 'build_to_spec', 'draw_circuit', 'draw_fix']); // drag/click-canvas/reorder/draw
const DRAW = new Set(['draw_circuit', 'draw_fix']);
...
let under15 = 0, noInteractive = 0, noImage = 0, noDraw = 0, pickerMajority = 0;
const rows = [];
...
for (const [id, lesson] of lessons) {
  ...
  const draws = graded.filter((s) => DRAW.has(s.type)).length;
...
console.log(`  lessons with < 15 graded steps:        ${under15} / ${lessons.length}`);
console.log(`  lessons with ZERO drawing steps:       ${noDraw} / ${lessons.length}`);
